...
class RSA:
  ...
  def do_RSA(self):
    ...
    result_file = open("./result_glove_vs_cls_doc.txt","a")
    ...
    for idx,batch in tqdm(enumerate(self.req_li),total=len(self.req_li)):
      ...
      for index in batch:
        ...
        try:
          ...
          #load glove embeddings for the doctsring
          with open(f'./Glove_embeddings/line_{index}.npy','rb') as f:
            glove_embed=np.load(f)
            f.close()
          ...
          glove_embed=glove_embed.reshape(-1).tolist()
          assert len(codeBERT_embed)==768
          assert len(glove_embed)==300


          #append list to form 2d matrix so that RSA can be done
          nl_list.append(glove_embed)
